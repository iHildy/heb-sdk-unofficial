...
Cookie sessions use authorized auth cookies from your session source.
...
```ts
import { createSessionFromCookies } from "heb-sdk"

const session = createSessionFromCookies(process.env.HEB_COOKIES ?? "")
```

```ts
import { createTokenSession } from "heb-sdk"
...
```ts
import { getSessionInfo, isSessionValid } from "heb-sdk"
