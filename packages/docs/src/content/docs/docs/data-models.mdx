...
These types are exported from `heb-sdk` for strong typing.
Use them to model inputs and normalize responses.

## Review sessions

- `HEBCookies`: required auth cookie values, optional `CURR_SESSION_STORE`, optional `shoppingContext`, plus extra keys
