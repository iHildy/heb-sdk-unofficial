...
Use only authorized session credentials and refresh them when they expire.
...
Use `createSessionFromCookies` for a Cookie header string or JSON cookie export.
Bearer sessions use OAuth tokens and a mobile user agent by default.

```ts
import { createSessionFromCookies, createTokenSession } from "heb-sdk"
...
```ts
import { updateTokenSession } from "heb-sdk"
...
```ts
import { getSessionInfo, isSessionAuthenticated, isSessionValid } from "heb-sdk"
