...
heb-auth is the recommended path because tokens can be refreshed on the server and don't require an active browser.
Bearer auth targets the mobile GraphQL host and is required for search, buy-it-again, product details, homepage, orders, account, and weekly ad.
