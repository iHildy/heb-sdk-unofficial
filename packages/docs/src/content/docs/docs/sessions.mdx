...
heb-auth-unofficial is the recommended path because tokens can be refreshed on the server and don't require an active browser like the cookie bridge.
...
:::caution
The cookie bridge extension is functional but no longer maintained. Use the `heb-auth-unofficial` package for the best experience.
:::
