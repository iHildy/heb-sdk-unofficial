    ...
    async ({ query, limit }) => {
      ...
      try {
        ...
        const formatted = products
          .map((p, i) => formatProductListItem(p, i))
          .join("\n");
      ...
      try {
        ...
        const details = [
          ...
          product.nutrition?.calories
            ? `\nNutrition: ${product.nutrition.calories} cal`
            : null,
    ...
    async () => {
      ...
      try {
        ...
        if (cart.fees.length > 0) {
          const feesList = cart.fees
            .map((fee) => `- ${fee.displayName}: ${fee.amount.formatted}`)
            .join("\n");
          feesSection = `\n\n**Fees:**\n${feesList}`;
        }
        ...
        if (cart.paymentGroups.length > 0) {
          const payments = cart.paymentGroups
            .map(
              (pg) =>
      ...
      try {
        ...
        const formatted = orders
          .map((order) => {
            ...
            const timeRange = ts?.formattedStartTime
              ? ` (${ts.formattedStartTime} - ${ts.formattedEndTime})`
              : "";
            const dateText =
              ts?.formattedDate ??
              (ts?.startTime
                ? new Date(ts.startTime).toLocaleDateString()
                : "Unknown date");
            const totalText =
              order.totalPrice?.formattedAmount ??
              (order as any)?.priceDetails?.total?.formattedAmount ??
              "Unknown total";
            const statusText =
              order.orderStatusMessageShort ??
              (order as any)?.status ??
              "Unknown status";
  ...
  server.tool(
    ...
    async ({ order_id }) => {
      ...
      try {
        ...
        const totalText =
          pageOrder?.priceDetails?.total?.formattedAmount ?? "Unknown total";

        const ts = pageOrder?.orderTimeslot;
        const timeText = ts?.formattedDate
          ? `${ts.formattedDate} (${ts.formattedStartTime} - ${ts.formattedEndTime})`
          : "Unknown";
      ...
      try {
        ...
        if (homepage.banners.length > 0) {
          ...
          homepage.banners.forEach((b, i) => {
            parts.push(
              `${i + 1}. ${b.title ?? "Untitled"}${b.linkUrl ? ` - ${b.linkUrl}` : ""}`,
            );
          });
        }
        ...
        if (homepage.featuredProducts.length > 0) {
          ...
          homepage.featuredProducts.forEach((p, i) => {
            ...
            parts.push(
              `${i + 1}. ${p.name}${p.brand ? ` (${p.brand})` : ""} ${price} (ID: ${p.productId})`,
            );
          });
        }
            ...
            if (s.items && s.items.length > 0) {
              s.items.forEach((item) => {
                ...
                // Check for banner/promo
                  ...
                  if (b.subtitle || b.description)
                    itemText += ` - ${b.subtitle ?? b.description}`;
                }
                // Fallback
                else {
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Found ${slots.length} curbside slots:\n\n${formatted}`,
            },
          ],
        };
      } catch (error) {
        ...
        const itemsList = list.items
          .map(
            (
              ...
              const priceStr = item.price?.total
  ...
  server.tool(
    ...
    async ({ limit, category_id, store_id, page_cursor }) => {
      ...
      try {
        ...
        const productsList = adResults.products
          .map((p, i) => {
        ...
        const summary = [
          ...
          adResults.validFrom && adResults.validTo
            ? `Valid: ${adResults.validFrom} to ${adResults.validTo}`
            : null,
  ...
  server.tool(
    ...
    async ({ store_id }) => {
      ...
      try {
        ...
        const categoriesList = adResults.categories
          .map((c) => `- ${c.name} (ID: ${c.id}) - ${c.count} items`)
          .join("\n");
