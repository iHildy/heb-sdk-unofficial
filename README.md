...
**Packages**
- `packages/heb-client` TypeScript SDK that wraps H‑E‑B web + mobile endpoints with typed models, formatters, and session helpers.
...
- `heb-mcp` stores sessions (local file or encrypted per‑user store) and exposes MCP tools backed by `heb-client`.
- `heb-auth` provides OAuth/PKCE helpers for linking mobile bearer tokens when needed.

**Extensibility**
- Reuse the existing MCP tool set as building blocks, or register your own tools by wiring new `heb-client` calls in `packages/heb-mcp/src/tools.ts`.
- Wrap the SDK however you want (for example, build a `heb-cli`, a cron worker, or a custom service layer).
- Extend API coverage in `packages/heb-client` (new endpoints, formatters, types) as your needs grow.
